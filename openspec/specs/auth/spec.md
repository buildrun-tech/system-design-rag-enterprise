...
Fornecer autenticação e autorização via AWS Cognito com provedores Google e GitHub, com emissão e validação stateless de JWT, para que apenas usuários autenticados acessem os recursos do sistema e cada requisição seja identificada de forma segura sem manter estado de sessão no backend.
