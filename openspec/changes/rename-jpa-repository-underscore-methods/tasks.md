...
- [x] 5.3 Rodar o skill `java-quality-gate` sobre os arquivos alterados: `run-coverage.sh` → `STATUS: PASS`, `LINE_COVERAGE: 96.25%`. `run-mutation.sh` (PITest) → `STATUS: PASS`, `MUTATION_SCORE: 86.54%` (rodado isolado, após outra sessão concorrente no mesmo repo ter corrompido uma tentativa anterior de `mutations.xml`).
